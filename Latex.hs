...
tikz_of_gate n (X i) = (1.5, code)
  where
    code x = [printf "\\gate{$X$}{%0.2f,%d};" x i]
tikz_of_gate n (Swap i j) = (1, code)
  where
...
assign_columns = aux Map.empty (-1)
  where
...
tikz_of_clifford_with_labels n gates l1s l2s = str
  where
...
tikz_of_example n cir = putStrLn s
  where
    ...
    ss =
      map
        ( \(x, y) ->
            tikz_of_clifford len x
              ++ tikz_of_clifford_with_labels len y ["|||||"] []
        )
        ns
    s = string_of_list "\\m{\n" "}&\\rightarrow &\n\\m{\n" "}\n" "" show ss
